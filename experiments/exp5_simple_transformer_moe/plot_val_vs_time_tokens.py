#!/usr/bin/env python3
"""
Plot validation loss vs TIME and vs TOKENS (the meaningful metrics!)
"""
import json
...
# Save plot
output_file1 = results_dir / 'val_loss_vs_time_proper.png'
...
# Save plot
output_file2 = results_dir / 'val_loss_vs_tokens_proper.png'
plt.savefig(output_file2, dpi=150, bbox_inches='tight')
print(f"📊 Saved: {output_file2}")
plt.close()

# ============================================================================
# PLOT 3: Combined - Val Loss vs Time AND Tokens side by side
# ============================================================================
fig, axes = plt.subplots(1, 2, figsize=(20, 8))
fig.suptitle('Validation Loss: Time vs Tokens Comparison - All Configurations', 
             fontsize=18, fontweight='bold', y=0.98)
...
# Save plot
output_file3 = results_dir / 'val_loss_time_vs_tokens_combined.png'
...
# ============================================================================
print("\n" + "="*80)
print("📊 VALIDATION LOSS ANALYSIS (Time & Tokens)")
print("="*80)

print(f"\n{'Config':<25} {'Time Range (s)':<18} {'Tokens Range (M)':<20} {'Val Loss Range':<20}")
print("-"*80)
...
for config_name in configs:
    ...
    data = all_data[config_name]
    if 'val_history' in data and data['val_history']:
        time_start = data['val_history'][0]['time']
        time_end = data['val_history'][-1]['time']
        tokens_start = data['val_history'][0]['tokens'] / 1e6
        tokens_end = data['val_history'][-1]['tokens'] / 1e6
        loss_start = data['val_history'][0]['val_loss']
        loss_end = data['val_history'][-1]['val_loss']
        
        print(f"{config_name:<25} {time_start:.1f} → {time_end:.1f}s      "
              f"{tokens_start:.2f} → {tokens_end:.2f}M      "
              f"{loss_start:.3f} → {loss_end:.3f}")

print(f"\n{'='*80}")
print("✅ All meaningful validation plots generated!")
print(f"{'='*80}\n")
