"""
Run systematic experiments to diagnose validation loss issues
"""
import argparse
import json
import time
...
def run_single_experiment(exp_name: str, output_dir: str = "./experiments"):
...
def run_multiple_experiments(exp_names: list, output_dir: str = "./experiments"):
...
def compare_experiments(results: dict, output_dir: str = "./experiments"):
    ...
    # Print summary table
    print(f"{'Experiment':<25} {'Final Loss':>12} {'Best Loss':>12} {'Final Acc':>12} {'Time (min)':>12}")
    print("-" * 80)
    ...
    for exp_name, data in results.items():
        ...
        print(f"{exp_name:<25} {metrics['val_loss']:>12.4f} {best_loss:>12.4f} "
    ...
    ax = axes[0, 0]
    for i, exp_data in enumerate(comparison_data):
        history = exp_data['history']
        ax.plot(history['steps'], history['val_losses'], 
                label=exp_data['name'], color=colors[i], linewidth=2, marker='o', markersize=3)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run systematic experiments to diagnose MoE training issues",
    ...
    parser.add_argument(
        '--quick',
        action='store_true',
        help='Run a quick test suite (baseline, early_stopping, lower_lb_weight)'
    )
    ...
    if args.all:
        ...
        exp_names = ['baseline', 'early_stopping', 'lower_lb_weight', 'no_lb']
        print(f"Running quick test suite: {exp_names}")
